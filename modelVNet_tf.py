...
def encoding_block(X, filter_size, filters_num, block_type, stage, s = 1, X_skip=0):
    ...
    kernel = tf.Variable(initializer(shape=(filter_size[0],filter_size[1],1,filters_num)), trainable=True)
    ...
    # Down sampling layer
    kernel = tf.Variable(initializer(shape=(2,2,1,filters_num*2)), trainable=True)
...
def decoding_block(X, filter_size, filters_num, block_type, stage, s = 1, X_jump = 0, up_sampling = True, output_shape = [1,1,1,1]):
    conv_name_base = 'conv_' + block_type + str(stage) + '_'
    bn_name_base = 'bn_' + block_type + str(stage)  + '_'
    ...
    if up_sampling == True:
        kernel = tf.Variable(initializer(shape=(2,2,1,filters_num)), trainable=True)
